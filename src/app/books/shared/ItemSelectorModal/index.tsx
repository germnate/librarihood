...
export function ItemSelectorModal({
  ...
  function createTag() {
    ...
    const newItem = { id: undefined, name: search, checked: true }
    console.log(newItem)
      ...
      <div className='flex flex-col mx-auto mt-12 px-2 py-2 bg-libraryGray sm:w-3/4 max-w-4xl opacity-100 rounded-lg shadow-lg text-gray-300 cursor-auto'>
        ...
        <div className='max-h-[75dvh] overflow-auto'>
          {stateItems.map((item, index) => {
            return (
              <div key={item.id || item.name} className={`flex justify-between px-4 py-2 bg-gray${index % 2 ? '-500' : '480'}`} onClick={() => updateChecked(item.name)}>
                <span>{item.name}</span>
